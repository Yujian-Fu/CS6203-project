...
def layer_analysis(agent_name_keys, adversarial_name_keys, updates, similarity_other_path, similarity_mean_path, write_header, epoch):
    for key in updates:
        for parameter_name in updates[key][1]:
            updates[key][1][parameter_name] = updates[key][1][parameter_name].cpu()
    ...
    for begnign_key in begnign_name_keys:
        ...
        for parameter_name in begnign_weight:
            ...
            cosine = np.sum(begnign_weight[parameter_name].numpy() * begnign_base_dict[parameter_name]) / np.power((np.sum(np.power(begnign_weight[parameter_name].numpy(), 2)) * np.sum(np.power(begnign_base_dict[parameter_name], 2))), 0.5)
            assert(cosine <= 1.0 and cosine >= -1.0)
